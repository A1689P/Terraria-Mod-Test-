        ...
        public override void Update(NPC npc, ref int buffIndex)
        {
            if(npc.active)
            {
                ...
                if(timer >= 60)
                {
                    ...
                    {
                        Vector2 dist = Npc.position - targetpos;
                        float distance = dist.Length();
                        if(distance <= 240)
                        {
                            if(Npc != npc)
                            {
                                Npc.AddBuff(ModContent.BuffType<Panic>(),120);
                                Npc.SimpleStrikeNPC(15,0);
                            }
                            else Npc.SimpleStrikeNPC(30,0);
                        }
                    }
                }
            }
